...
//@Service Annotation to indicate that a class belongs to the service layer where business logic is written.
@Service
public class UserGiftDetailsAccountManagementServiceImpl implements IUserGiftDetailsAccountManagementService {
	...
	@Override
	public UserGiftDetails addUserGiftDetails(UserGiftDetails userGiftDetails) throws DuplicateUserGiftDetailsIdException {
		Optional<UserGiftDetails> userDet = userGiftDetailsRepo.findById(userGiftDetails.getUserGiftId());
		if (userDet.isPresent()) {
			throw new DuplicateUserGiftDetailsIdException(
...
//		userGiftDetails.setPersonalize(null);
	...
	/*
	@Override
	public List<UserGiftDetails> getUserGiftOrderHistory(int id) throws UserGiftDetailsNotFoundException {
